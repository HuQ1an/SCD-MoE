...
def SCDD_eval_all_filtered(preds, labels, num_class=26):
    # 1. 基础混淆矩阵构建 (26x26)
    hist = np.zeros((num_class, num_class))
    ...
    # 2. 计算 IoU_fg (保持原样，评价二值检测能力)
    # 即使是无效类别的变化，只要位置对了，IoU_fg 应该给分
    hist_fg = hist[1:, 1:]
    c2hist = np.zeros((2, 2))
    c2hist[0][0] = hist[0][0]
    ...
    # --- 【补丁核心】 ---
    # 定义需要保留的有效 ID (Valid IDs)
    # 我们要从 0~25 中剔除掉涉及 A=4 或 B=3 的 ID
    all_indices = np.arange(num_class)
    
    # 手动列出无效 ID (根据 A缺4, B缺3 推算)
    # A=4 (16-20), B=3 (3,8,13,18,23)
    invalid_ids = [3, 8, 13, 16, 17, 18, 19, 20, 23]
    
    # 无论如何，ID 0 (背景) 必须保留在矩阵里用于后续 kappa_n0 去除操作
    valid_indices = [i for i in all_indices if i not in invalid_ids]
    
    # 提取子矩阵 (Filtering)
    # 形状会从 26x26 变成 17x17 (26 - 9 = 17)
    hist_filtered = hist[np.ix_(valid_indices, valid_indices)]

    # 3. 计算 Kappa_n0 (基于过滤后的矩阵)
    # 复制一份用于操作
    hist_n0 = hist_filtered.copy()
    
    # 只要 valid_indices[0] 是 0，这里就是安全的
    # 也就是移除 "背景-背景" 的计数，专注于 "变化-变化" 和 "背景-变化" 的错误
    hist_n0[0][0] = 0 
    ...
    # --- Fscd 计算 (建议也基于 Filtered，或者保持原样) ---
    # Fscd 关注的是变化的探测率，建议保持用原始 hist 或者 filtered 均可
    # 这里为了统一，展示用原始 hist 的逻辑
    pixel_sum = hist.sum()
    change_pred_sum = pixel_sum - hist.sum(1)[0]
    change_label_sum = pixel_sum - hist.sum(0)[0]
    SC_TP = np.diag(hist[1:, 1:]).sum() # 这里包含了无效类的TP(如果有的话)
    
    eps = 1e-10
    SC_Precision = SC_TP / (change_pred_sum + eps)
    SC_Recall = SC_TP / (change_label_sum + eps)
...
def SCDD_eval_all(preds, labels, num_class):
    ...
    eps = 1e-10
    if change_pred_sum <= 0 or change_label_sum <= 0 or SC_TP <= 0:
        Fscd = 0.0
    else:
        SC_Precision = SC_TP / (change_pred_sum + eps)
        SC_Recall = SC_TP / (change_label_sum + eps)
        # Use stable F1-style harmonic mean to avoid scipy.stats.hmean restrictions
        Fscd = stats.hmean([SC_Precision, SC_Recall])
    ...
    # print("\n====== 混淆矩阵诊断 (Rows=GT, Cols=Pred) ======")
    # # 我们只看几个关键的 ID
    # # 假设: 
    # # ID 0 = 背景
    # # ID 25 = 5变5 (如果公式是 A*6+B) 或 (5-1)*5+5
    # # ID 30 = 5变0 (如果预测成背景)
    # # ID 24 = 预测了 4变4 (如果是训练偏移导致)
    ...
    #     # 只打印数量比较多的项，忽略噪声
    #     if hist[r, c] > 100: 
    #         print(f"真值 ID: {r} --> 预测 ID: {c} | 像素数: {hist[r, c]}")
